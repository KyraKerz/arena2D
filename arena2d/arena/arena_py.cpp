...
PyObject* Arena::packPyObservation(int env_index)
{
	...
	if(_level != NULL){
		_level->getAgentData(_burger->getBody()->GetTransform(), additional_data);
	}
...
PyObject* Arena::packAllPyActions()
{
	PyObject * actions;
	if(_numEnvs ==1)
	{
		actions = PyLong_FromLong(_actions[0]);
	}else{
		actions = PyList_New(_numEnvs);
		for(int i = 0; i < _numEnvs; i++){
			PyList_SetItem(actions, i, PyLong_FromLong(_actions[i]));
		}
	}
	return actions;
}
