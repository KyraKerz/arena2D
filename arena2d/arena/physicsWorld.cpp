...
const b2Vec2 PHYSICS_NEIGHBOUR_MAP_DIAGONAL[4] = {b2Vec2(1, 1), b2Vec2(-1,1), b2Vec2(-1, -1), b2Vec2(1, -1)};
...
{
	_world->Step(_timeStretch*time_step, _velocityIterations, _positionIterations);
}
...
void PhysicsWorld::debugDrawWorld(b2World * w, unsigned int flags, uint16 category_bits)
{
	...
	for(;it != it_end; it++)
	{
		...
		case b2Shape::e_circle:
		{
			...
			Z_SHADER->setModelviewMatrix(modelview);
			Z_SHADER->setColor((float*)&color);
			_RENDERER->bindGO(GO_CIRCLE_SMOOTH);
			if(setColorplex)
				colorplex->setPatternType(colorplex_type);
			_RENDERER->drawGO();
			if(setColorplex)
				colorplex->setPatternType(Colorplex2DShader::NONE);
			Z_SHADER->setColor((float*)&dark);
			_RENDERER->bindGO(GO_CIRCLE_SMOOTH_LINE);
			_RENDERER->drawGO();
		}break;
		case b2Shape::e_edge:
		{
