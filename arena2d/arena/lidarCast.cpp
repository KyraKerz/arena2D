...
LidarCast::LidarCast(int num_samples, float max_distance, float start_angle, float end_angle, float noise, b2Body * filter_body):  _numSamples(num_samples), _noise(noise), _startAngle(start_angle), _endAngle(end_angle),  _filterBody(filter_body), _maxDistance(max_distance)
{
...
{
	_closestPoint = -1;
	float angle_i = 2*M_PI/_numSamples;
