...
class Agent:
	def initialize(self, device, model_name, agent_name):
		...
		self.tensor_reward_buffer = torch.zeros(MEMORY_SIZE, dtype=torch.float).to(device)# rewards
		...
		self.net = fc.FC_DQN(NUM_INPUTS, NUM_ACTIONS)
		self.tgt_net = fc.FC_DQN(NUM_INPUTS, NUM_ACTIONS)
		#self.net = cnn_fc.CNN_FC_DQN(NUM_ACTIONS)
		#self.tgt_net = cnn_fc.CNN_FC_DQN(NUM_ACTIONS) 
		# copy to device
	...
	def pre_step(self, observation):
		...
		if random.random() <= self.epsilon: # random action
			...
			state_v_batch = self.tensor_state_buffer[idx].view(1, NUM_INPUTS)
...
def post_step(new_observation, action, reward, mean_reward, is_done, mean_success):
	...
	a = agent.post_step(new_observation, action, reward, mean_reward, is_done, mean_success)
	return a
