#ifndef F_GLOBAL_SETTINGS_H
#define F_GLOBAL_SETTINGS_H
...
public:
    ...
    void init(const char * file_name = DEFAULT_SETTINGS_FILE_NAME);
    ...
    int saveToFile(const char * path = DEFAULT_SETTINGS_FILE_NAME);
	int save(){return saveToFile(((_lastSettingsPath == NULL) ?DEFAULT_SETTINGS_FILE_NAME : _lastSettingsPath));};

    //load settings from file
    //if @silence set to 1 than no error messages are printed via log
