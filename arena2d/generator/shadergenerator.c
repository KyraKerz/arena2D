...
{	
	...
	if(argc == 4 || argc == 3)//argument is given
	{
		...
		char frag_file[BUFFER_SIZE];
		char vert_file[BUFFER_SIZE];
		int frag_len = 0;
		int vert_len = 0;
	...
	{		
		...
		if(c == '\n' || c == '\r' )
		{
			if(!(c == '\n' && last_c == '\r'))//already did a linebreak
			{
				...
				{
				printf("include directive found, path: %s, relative_path: %s\n", include_path, relative_path);
					res = loadFile(include_path, &text[last_linebreak_index-1]);
					if(res < 0)
					{
						fprintf(stderr, "Include directive in line %d failed: #include \"%s\"\n", line_count, include_path);
						return -1;
					}
...
int checkIncludeDirective(const char * line, int line_size, int num_line, const char * relative_path, char * include_path)
{
	...
	while(line[count] == ' ' || line[count] == '\t')//removing pre white characters
	{
		...
		if(count >= line_size)
		{
			fprintf(stderr, "line %d: '%.*s' missing path!\n", num_line, line_size, line);
			return -1;
		}
	}
	...
	if(line[count] != '"')
	{
		fprintf(stderr, "line %d: '%.*s' expected '\"'!\n", num_line, line_size, line);
		return -1;
	}
	count++;
	if(count >= line_size)
	{
		fprintf(stderr, "line %d: '%.*s' expected string after '\"'!\n", num_line, line_size, line);
		return -1;
	}
