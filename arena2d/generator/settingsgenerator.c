...
char * DATA_TYPE_NAMES[4] = {"int", "float", "char", "SDL_Keycode"};
...
int main(int argc, char ** argv)
{
	...
	int last_line = 0;//line-index of last iteration
	...
	while(readNextWord(word, BUFFER_SIZE))
	{
		...
		if(comment_count == 2)
		{
			if(last_line == currentLine)//line comment still goes on
				skip = 1;
			else
				comment_count = 0;//end of line (comment)
		}
		if(comment_count == 0)
		{
			if(strcmp(word, "/") == 0)
