...
#include <string>
const std::string GLOBAL_PACKAGE_PATH = "/home/joe/projects/rl/src/arena2d-sim";
...
void GlobalSettings::init(const char * path)
{
...
void GlobalSettings::setToDefault()
{
	...
	_settings.video.msaa = 0;
	_settings.video.enabled = 1;

	// gui
	_settings.gui.font_size = 14;
	...
	// training
	_settings.training.max_time = 60.0f;
	_settings.training.episode_over_on_hit = 0;
	_settings.training.reward_goal = 100.0f;
	_settings.training.reward_distance_to_goal_decreased = 0.1f;
	_settings.training.reward_distance_to_goal_increased = -0.2f;
	_settings.training.reward_wall = -10.0f;
	_settings.training.reward_time_out = 0.f;
	_settings.training.num_envs = 1;
	_settings.training.num_threads = -1;

	// ros
	_settings.ros.enabled = 0;

	// stage
	_settings.stage.initial_level = 5;
	...
	_settings.stage.enable_position_reset = 0;
	_settings.stage.goal_size = 0.1;
	strcpy(_settings.stage.svg_path, "level/examples/");
...
int GlobalSettings::loadFromString(const char * c)
{
	...
	buffer[0] = '\0';
	int buffer_len = 0;
	static const char * breakSymbols = "{} =\0\n\t;";
	int num_breakSymbols = strlen(breakSymbols);
	...
	while(!error)
	{
		...
		case 3:
		{
			...
			else if(last_line < current_line){
				mode = 0;
			}else
			{
...
int GlobalSettings::saveToFile(const char * path)
{
	FILE * f = fopen(path, "w");
	if(f == NULL)
	{
