...
Console::Console(zFont * font, zFontSize size, const char * path_command_history)
{
	...
	float t_color[3] = {0.9f, 0.9f, 0.9f};
	_commandLine = new zTextView(font, size, CONSOLE_MAX_CHARS +_preTextLength +1);
	_commandLine->setColor(t_color[0], t_color[1], t_color[2]);
	_cursor = new zTextView(font, size, "_");
	_cursor->setColor(t_color[0], t_color[1], t_color[2]);
	_currentCommand[0] = '\0';
	...
	if(path_command_history != NULL){
		...
		if(f == NULL){
			WARNING_F("Unable to open command history '%s': %s", path_command_history, strerror(errno));
		}
		else{
